...
for run in range(n_runs):
    ...
    # If no splits found for this run it prints this message, the full RF needs to be trained first to establish the splits used by the DT and three feature RF
    ...
    # Ensures all columns are numeric, should not be any categorical columns for this DT model
    ...
    # Uses the models to predict on the X_test subset, this was created in the full RF training script and exists in the splits folder
...
if len(results["conf_matrices"]) == 0:
    ...
    # Computs mean and standard deviation of class level metrics across runs
