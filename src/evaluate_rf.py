...
# Ensures that new data sets match the models' training feature structure
# Adds missing columns and fills them with NaNs, which are then handled by the preprocessing pipeline
...
def summarize_per_model(cms, accs, bals, kaps, aucs, precs, recs, f1s):
    ...
    print(f"No Damage(0) Precision: {precs[:,0].mean():.3f} ± {precs[:,0].std():.3f}")
    print(f"No Damage(0) Recall: {recs[:,0].mean():.3f} ± {recs[:,0].std():.3f}")
    print(f"No Damage(0) F1: {f1s[:,0].mean():.3f} ± {f1s[:,0].std():.3f}")

    print(f"Damage(1) Precision: {precs[:,1].mean():.3f} ± {precs[:,1].std():.3f}")
    print(f"Damage(1) Recall: {recs[:,1].mean():.3f} ± {recs[:,1].std():.3f}")
    print(f"Damage(1) F1: {f1s[:,1].mean():.3f} ± {f1s[:,1].std():.3f}")
...
for state in states:
    ...
    # TX uses the ratio 59 damage : 16 no damage
    # Others use 1 damage : 5 no damage
    if state == "TX":
    ...
    df["p_mean_adj"] = expit(logit_p + (logit(pi_pop_state) - logit(pi_train)))


    cms, accs, bals, kaps, aucs, precs, recs, f1s = [], [], [], [], [], [], [], []
