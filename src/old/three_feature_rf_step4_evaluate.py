...
# Directory, paths to results and states to analyze
...
for state in states:
    file = os.path.join(results_dir, f"{state}_three_feature_predictions.csv") # csv with ensemble statistics
    all_probs_file = os.path.join(results_dir, f"{state}_three_feature_all_probs.npy") # NumPy array with per-model probabilities

    # Error message if prediction files are missing
    if not os.path.exists(file) or not os.path.exists(all_probs_file):
        print(f"No predictions for {state}, skipping")
        continue

    # Loads true damage lables, converts them to binary, 0 for no damage and 1 for damage
    df = pd.read_csv(file)
    y_true = df['Damage_Status'].map({'No Damage': 0, 'Damage': 1}).values

    # Loads per-model probabilities, ensure each is the right shape
    ...
    for i in range(n_models):
        y_pred = (all_probs[i] > 0.5).astype(int) # Decision threshold for damage (probability of damage above 0.5)
        ...
        # Class level performance metrics, precision, recall, and F1
