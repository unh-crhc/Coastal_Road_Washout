...
for run in range(n_runs):
    ...
    # Loads test features and keeps only the 3 input features
    ...
    if not os.path.exists(model_file):
        print(f"Warning: {model_type} model for run {run} missing. Skipping.")
        continue
    ...
    # Computes class level performance metrics
    ...
    # Prints a completion note for each run
