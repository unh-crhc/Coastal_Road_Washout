...
The processed datasets include predictor variables representing roadway characteristics, topography, and storm exposure. These features were selected to capture the primary drivers of coastal roadway damage and to ensure consistency across study areas. While roadway attribute availability varies by state due to differences in publicly available GIS data, all datasets share a common set of topographic and storm exposure variables.

Prior to modeling, damage locations were matched to roadway segments, comparison (no-damage) segments were selected, and predictor variables were compiled into state-level datasets. Derived variables, such as relative elevation and inundation duration, were calculated during these preprocessing steps. When a roadway attribute was unavailable for a given state, the feature structure was preserved by imputing missing values (median for numerical variables and mode for categorical variables), ensuring consistent model inputs across all datasets.

The predictor variables in the processed datasets are listed below.
